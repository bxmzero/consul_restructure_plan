...
所有由 Java 重构得到的 Go 函数、方法或接口方法，原则上应与对应 Java 函数保持契约一致。
...
- 入参含义、数量和必填/可选语义
- 出参含义和返回值语义
- Java 异常到 Go `error` 的映射语义
- 空值、默认值、边界值和失败行为
- 副作用，包括数据库写入、缓存、通知、日志和状态变更

允许因为 Java 和 Go 的语言差异做必要调整，例如：

- Java exception 转为 Go `error`
- Java nullable 对应 Go pointer、zero value 或 `sql.Null*`
- Java overload 拆成明确命名的 Go 函数
- Java class method 转为 Go interface method、struct method 或 package function
- Java collection / generic 类型转为 Go slice、map 或显式类型

任何偏离 Java 函数契约的调整都必须记录：
...
### 3.7 Java 与 Go 文件映射规则

本节及后续 3.8、3.9、3.10 的可追溯规则是整个迁移项目的全局强制规则，适用于全部六个阶段：
